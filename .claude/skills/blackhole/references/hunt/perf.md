...
| Full-table scan / unindexed sort (Path 1) | Read-verified scan or sort without index/key path where one exists in a hot path | 5–8 | 3–5 | MEDIUM–HIGH | A queue sync helper sorts the full open-issue list in memory on every orchestrator turn despite a keyed lookup table (illustrative, invented) → gain 7, effort 4, severity HIGH → Priority 7 × (11 − 4) = 49 (moderate) |
| Repeated expensive call in loop (Path 1) | Read-verified identical expensive call inside a loop without memoization/caching | 5–8 | 2–4 | MEDIUM–HIGH | A build step re-parses the same manifest file on every target iteration inside a loop (illustrative, invented) → gain 6, effort 3, severity HIGH → Priority 6 × (11 − 3) = 48 (moderate) |
| Nested loop vs map lookup (Path 1) | Read-verified nested loop over a full collection where a map/index lookup would suffice | 4–7 | 3–5 | MEDIUM | A dedup pass uses nested loops over findings instead of a keyed Set/Map (illustrative, invented) → gain 5, effort 4, severity MEDIUM → Priority 5 × (11 − 4) = 35 (borderline) |
...
<!-- GENERATED by scripts/build.ts from src/references/hunt/perf.md — do not hand-edit -->
