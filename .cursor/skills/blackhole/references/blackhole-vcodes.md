...
| V-DOCHEALTH-01 | A root `documentation/INDEX.md` row resolves to no existing, eligible file (dangling row) | BLOCK | scripts/checks/doc-health.check.ts |
| V-DOCHEALTH-02 | An eligible doc under `documentation/` has no corresponding root `documentation/INDEX.md` row (orphan file) | BLOCK | scripts/checks/doc-health.check.ts |
| V-DOCHEALTH-03 | Aggregated doc-tree health signal (single-doc line ceiling, root-INDEX row ceiling, tree-size advisory, deprecated-doc window) — always advisory, never blocking | WARN | scripts/checks/doc-health.check.ts |
| V-DOCHEALTH-04 | Root documentation/INDEX.md row content (summary/type/status/review_trigger) diverges from the doc's own frontmatter for a path present in both committed and generated output | BLOCK | scripts/checks/doc-health.check.ts |
...
| V-DOC-06 | Incident archaeology in source comments — issue/PR number, "found by review of X", or change-history prose embedded in a comment (a regression test may carry its issue number in the **function name** only). Three settled boundaries: (1) `describe()`/`test()` titles are exempt exactly like function names — a regression test may name its issue in its title; (2) module-header comments: the `#N` tag is archaeology and is removed, the load-bearing rationale stays and is reworded to state the invariant without the citation; (3) markdown prose under `src/**/*.md` and `documentation/**` is exempt — doc prose citations follow the established convention | WARN | reviewer.md § Comment Discipline Audit |
...
| V-ADR-04 | An ADR whose frontmatter `status:` is `superseded` is not also `superseded` in `documentation/decisions/INDEX.md`, or carries no supersession citation (`superseded_by:`/`supersedes: ADR-NNN` frontmatter, or a `Superseded by ADR-NNN` mention in the body) — the ADR-file leg of `V-ADA-08`'s lifecycle rule, emitted under its own id | WARN | scripts/checks/adr-status.check.ts |
| V-ADR-05 | Two `documentation/decisions/ADR-NNN-*.md` files claim the same `NNN` — the next-free-number collision that makes a fresh ADR overwrite an accepted one; a gap in the sequence is reported as advisory detail only, never a failure | WARN | scripts/checks/adr-status.check.ts |
...
| V-ADR-08 | ADR heading shape does not match either `ADR_SHAPES.classic` (5-heading narrative record) or `ADR_SHAPES.designTrack` (8-heading plan-track-gated design note) exactly — advisory by decision, not by omission (#741): does not block merge, pre-existing corpus grandfathered; revisit after V-CONTENTGATE-03 (#722) lands the grandfather-allowlist precedent | WARN | scripts/checks/adr-shape.check.ts |
...
| V-GATE-01 | A required gate marker went missing from an agent prompt — `implementer.md`'s 5-step verification-evidence gate or `reviewer.md` § Suggestion Proportionality Gate no longer carries its declared marker text, so the gate can be edited away without any check noticing | WARN | scripts/checks/agents.check.ts |
...
| V-SHAPE-01 | route field-set parity, two legs — (1) router.ts's `validateRoute` `requireField` keys vs. `campaign-status/types.ts`'s `Route` type keys diverge without a declared `omits:` allowlist entry on `Route`'s header comment (undeclared drift in either direction); (2) every `<!-- shape: exhaustive -->`-marked fenced `route` JSON example under `src/references/*.md` (currently `worker-schemas.md`, `queue-dag.md`) must match router.ts's required key set exactly, no `omits:` allowlist applies to this leg | WARN | scripts/checks/route-shape.check.ts |
...
| V-INCLUDE-02 | A file declared in `INCLUDE_MARKER_SITES` does not exist or carries no `{{INCLUDE:<dir>/*}}` marker, or a live marker under `src/**` sits at a file not declared in `INCLUDE_MARKER_SITES` | BLOCK | scripts/checks/build-input-dirs.check.ts |
| V-AUDIT-01 | Reviewer audit-module registry drift — a V-code whose primary enforcement site names `reviewer.md` is claimed by no `src/references/audits/*.md` `vcodes:` list or by more than one, a module claims a code with no row in this table, or the module count diverges from `REVIEWER_AUDIT_MODULE_COUNT` | BLOCK | scripts/checks/audit-modules.check.ts |
| V-CONTENTGATE-01 | Declared content-gate budget exceeded — a target's largest section (boundary unit per `CONTENT_GATE_BOUNDARY_UNITS`) or its whole-file LOC is over its `CONTENT_GATE_BUDGETS` glob-class ceiling, or over its `CONTENT_GATE_GRANDFATHERED` ceiling where one is declared; a `.check.ts` target detecting zero sections fails here too, since its section budget would otherwise be silently unenforced | BLOCK | scripts/checks/content-gates.check.ts |
| V-CONTENTGATE-02 | Content-gate exhaustion advisory — a section or whole file has crossed `CONTENT_GATE_WARN_RATIO` of its ceiling without yet exceeding it | WARN | scripts/checks/content-gates.check.ts |
| V-CONTENTGATE-03 | Content-gate ceiling exception unbacked — a `CONTENT_GATE_GRANDFATHERED` entry whose `sunset_adr` is not `ADR-NNN`-shaped or resolves to no `documentation/decisions/INDEX.md` row, or whose ceiling no longer exceeds its glob-class budget on either metric (the residue a class-budget raise that swallowed the exception leaves behind) | WARN | scripts/checks/content-gates.check.ts |
