...
| V-TEST-09 | Coverage regression on changed files — line/function coverage vs. pre-change baseline must not drop | BLOCK | implementer.md § Verification Evidence Gate (Coverage-regression gate) |
...
| V-ADA-01 | `ARCHITECTURE.md` absent at project root/package (remedy: create from template) | WARN | reviewer.md §10 (Companion-File Audit) |
...
| V-DOC-GOV-01 | New doc created under `documentation/` without a search-before-write check (duplicate-concern risk) | WARN | doc-governance.md § Search-Before-Write |
