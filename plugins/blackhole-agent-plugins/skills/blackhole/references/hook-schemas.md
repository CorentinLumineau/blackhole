...
`outside-assigned-worktree` fires from either of two resolution paths to the same `pattern_id` — one declared, one derived — both reaching `readAssignedWorktreeRoot(cwd)` in `hook-event-log.js`:

- **Declared (#620, Pattern B / manual use):** `BLACKHOLE_ASSIGNED_WORKTREE` is set to a registered family worktree, and the Write/Edit target resolves outside that single assigned root.
- **cwd-derived (#907, the mechanism Pattern C actually reaches):** the env var is unset, empty, or not a registered family worktree, and the PreToolUse payload's own `cwd` resolves to a linked (non-main) worktree — that worktree (or the validated scratchpad root it nests under, when one is configured) becomes the sole assigned root automatically, no declaration needed. See `orchestrator-dispatch.md` § Implementer write-root containment for why this is the reachable mechanism under the native `Agent`/`Workflow` tool, where the declared env var never survives to a spawned worker at all.

Containment falls back to `allWorktreeRoots(cwd)` unchanged (fail-open, the same degradation as an invalid `scratchpad_dir` in #510) only when neither path narrows: `cwd` resolves to the main clone itself (the orchestrator's/`planner`'s/`investigator`'s/`hunter`'s legitimate case), or a validated scratchpad candidate is present but does not nest the resolved worktree — deferred deliberately rather than silently dropped, since a single assigned-root string cannot represent two disjoint trusted roots at once.
...
`bash-outside-assigned-worktree` and `bash-write-target-unresolvable` (issue #804, ADR-029) extend `outside-assigned-worktree`'s containment to the `Bash` tool: `validate-bash-command.js`'s `bash-write-target-guard.js` extracts common file-write-target shapes (`>`, `>>`, `&>`, `tee [-a]`, `sed -i[.suffix]`, `cp`, `mv`, heredoc targets) from the command string and, whenever `readAssignedWorktreeRoot(cwd)` resolves a non-null root — from either the declared `BLACKHOLE_ASSIGNED_WORKTREE` leg or the cwd-derived tier above (#907) — checks each resolvable target against that same single root. A resolvable target outside the assigned root denies (`bash-outside-assigned-worktree`, block tier); a write-shaped command whose target cannot be resolved statically (a dynamic argument, or a command like `python3 -c`/`perl -i`/`awk`/`dd`/`rsync` whose write behavior cannot be determined from the command string alone) is allowed but recorded (`bash-write-target-unresolvable`, warn tier) — never a silent allow. Only when `readAssignedWorktreeRoot(cwd)` itself resolves null (`cwd` is the main clone, or a non-nesting scratchpad candidate deferred the decision) does this check fall open, byte-identical to today (same #510 `allWorktreeRoots` degradation `outside-assigned-worktree` already documents) — this is no longer simply "env var unset".
