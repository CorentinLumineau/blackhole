...
| V-SHAPE-01 | route field-set parity — router.ts `requireField` keys vs. `campaign-status/types.ts` `Route` keys diverge without a declared `omits:` allowlist entry (undeclared drift); or a doc marked `<!-- shape: exhaustive -->` in `worker-schemas.md`/`queue-dag.md` omits a router-required field | WARN | scripts/checks/route-shape.check.ts |
