...
`gated-batch` (within § 4's per-issue sequential loop). No longer bypassed under `merge_mode:
leave-open` (ADR-026 D5 narrowed the old blanket bypass): this section now also runs for
`leave-open` PRs as part of that mode's merge-readiness dry run, and again as a targeted restack
when `merge-gate.md` § 3 observes a `leave-open` PR merged externally
(`merge-gate.md` § 3 "Restack on observed human merge", D6) — rescanning the campaign's other
open PRs against `main`'s new HEAD and running this same protocol per PR that would now conflict.
Only the terminal `gh pr merge` call and `mergeEligible()`'s admin-scheduling conditions remain
bypassed for `leave-open`; a human still merges the PR itself manually.
...
| `merge_mode: leave-open` | Preflight runs as a merge-readiness dry run (ADR-026 D5) and as a post-external-merge sibling restack (D6) — but the campaign never calls `gh pr merge` itself; a human still merges the PR manually once ready |

## Consulted by

- `phase-loop.md` § Merge protocol, Step 0.5 (between step 0 and step 1, including under
  `merge_mode: leave-open`'s merge-readiness dry run, ADR-026 D5).
- `merge-gate.md` § 3 "Restack on observed human merge" (D6) — a targeted re-run against sibling
  PRs after an externally-observed `leave-open` merge.
...
None of these five files duplicate the algorithm above inline — they cite this doc by pointer.
<!-- GENERATED by scripts/build.ts from src/references/merge-conflict-protocol.md — do not hand-edit -->
