...
| `tier` | `block` \| `warn` | yes |
| `pattern_id` | matched entry's `id`, or `outside-worktree` \| `outside-cwd-fallback` \| `cwd-fallback-too-broad` \| `pattern-load-failure` \| `hook-input-parse-failure` \| `uncaught-validator-error` | yes |
