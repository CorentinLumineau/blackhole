...
| `stop_kind` | checkpoint frontmatter | `drained` \| `killed` \| `flushed` (issue #492) \| `null` — which `phase-stop.md` tier ran; `flushed` is emitted per that file's `stop --now` tier step 6 priority rule |
| `worker_state` | `## In-flight workers` row, stop-event-only | `drained` \| `flushed` \| `killed` — tags a stopped worker's row before removal per `phase-stop.md`'s § `stop --now` tier step 3 (`flushed`) and § `--abandon` tier (`killed`); absent on a normal turn's rows |
