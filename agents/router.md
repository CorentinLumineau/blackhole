...
Dispatch of the resulting flag is additionally gated by `autonomy.analyze_routing` — see
`config-template.md` — a config concern this agent does not implement, only computes the flag
for.
...
`route.ui` is content-derived from the issue, same as `docs_impact` above (`V-INT-03` — no new
heuristic shape). Classify `true` if the issue's expected diff touches a UI/frontend surface —
...
`docs_impact` (an unreviewed UI merge is the costlier failure mode than one extra human
round-trip).
