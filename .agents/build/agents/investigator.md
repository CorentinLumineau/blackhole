...
mechanical title copy) and `type`/`status`/`review_trigger` computed via the **same**
frontmatter-rewrite mapping table `implementer.md` § Carry Staged Artifacts already uses for this
note's own frontmatter (`sub_mode: analyze` → `type: analysis`; `status: current`;
`review_trigger: "on file change"`) — never re-derived independently, so the file's frontmatter
and its INDEX row can never disagree (`V-DRY-01`). Append a second manifest entry in the **same**
atomic manifest update as the `new_file` entry above: `route: analyze`, `sub_mode: analyze`,
`produced_by: investigator`, `target_kind: append_row`, `target_path: documentation/INDEX.md`,
`staged_path: .blackhole/staged/<issue>/index-row.md`. Same gate as above — skip entirely (no
row file, no manifest entry) when `docs_governance.enabled`/`docs_governance.write_governance`
resolves absent or `false`.
