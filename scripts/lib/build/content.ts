...
// `srcDir` (src/) — the same anchor `BUILD_INPUT_ONLY_DIRS` declares its entries against, so
// expandIncludes and compileFolder's exclusion below never need two different path conventions.
// `<`/`>` excluded from the captured group: real directory names never contain them, and this
// keeps a doc's own illustrative `{{INCLUDE:<dir>/*}}` placeholder text (e.g.
// blackhole-vcodes.md's own V-INCLUDE-01 row, itself a compiled src/references/*.md file) from
// being mistaken for a real marker. Exported so build-input-dirs.check.ts's independent scan
...
export const expandIncludes = (
  ...
  content.replace(INCLUDE_MARKER, (_match, dirRel: string) => {
    ...
    if (files.length === 0) {
      throw new Error(
        `expandIncludes: {{INCLUDE:${dirRel}/*}} in ${path.relative(root, srcPath)} names a directory that does not exist or has zero .md files: ${dirRel}`
      );
    }
...
// `extraSources` (ADR-034) names every {{INCLUDE:<dir>/*}}-inlined module path, so a compiled
// file with build-time-composed content never under-reports its real inputs. Defaults to `[]`,
// which reproduces today's exact marker string byte-for-byte — every pre-#719 call site is
// unaffected.
export const generatedMarkerLine = (
  relSrcPath: string,
  style: 'html' | 'yaml',
  extraSources: string[] = []
): string => {
  const sourcesText = extraSources.length ? ` (includes: ${extraSources.join(', ')})` : '';
  const text = `GENERATED by scripts/build.ts from ${relSrcPath}${sourcesText} — do not hand-edit`;
...
export const processFile = (
  ...
  const originalContent = expandIncludes(fs.readFileSync(srcPath, 'utf-8'), srcPath, extraSources);
  ...
  if (isAgent && target === 'codex') {
    ...
    const markedYaml = `${yaml}\n${generatedMarkerLine(relSrcPath, 'yaml', extraSources)}\n`;
  ...
  const marked = `${compiled}${separator}${generatedMarkerLine(relSrcPath, 'html', extraSources)}\n`;

  fs.writeFileSync(destPath, marked, 'utf-8');
};

// ADR-034: true when `srcPath` (absolute) falls under a declared BUILD_INPUT_ONLY_DIRS entry —
// compared relative to `srcDir`, the same anchor expandIncludes resolves a marker's <dir>
// against, so a declared entry means the same directory to both sides of the primitive.
const isBuildInputOnlyPath = (srcPath: string): boolean => {
  const relToSrcRoot = path.relative(srcDir, srcPath).split(path.sep).join('/');
  return BUILD_INPUT_ONLY_DIRS.some(
    (dir) => relToSrcRoot === dir || relToSrcRoot.startsWith(`${dir}/`)
  );
};
...
export const compileFolder = (srcSub: string, destParent: string, agentDir: string, rulesPath: string, target: Target, isAgent = false) => {
  ...
  for (const srcPath of walkFilesAbs(fullSrc)) {
    // ADR-034: a declared build-input-only directory is a build input consumed only via
    // {{INCLUDE:<dir>/*}} — it must never also be mirrored into a compiled output tree.
    if (isBuildInputOnlyPath(srcPath)) continue;
