...
import { AGENT_NAMES, PLATFORM_TARGETS, BUILD_INPUT_ONLY_DIRS, INCLUDE_MARKER_SITES, RULES_LIST } from './lib/build/facts.ts';
...
describe('cleanDir(.claude/skills) narrowed to blackhole/ only (issue #267)', () => {
  const runBuild = (): { status: number | null } => {
    const proc = Bun.spawnSync({
  ...
  test('a sibling maintainer skill under .claude/skills/ survives a rebuild, and blackhole/SKILL.md is still regenerated', () => {
    const sentinelDir = path.join(root, '.claude', 'skills', '_sentinel_test');
    const sentinelFile = path.join(sentinelDir, 'SKILL.md');
    const sentinelContent = '# sentinel — must survive scripts/build.ts\n';
    fs.mkdirSync(sentinelDir, { recursive: true });
    fs.writeFileSync(sentinelFile, sentinelContent);

    try {
      const result = runBuild();
      expect(result.status).toBe(0);

      // Sentinel: build.ts must not have touched a skill dir it doesn't own.
      expect(fs.existsSync(sentinelFile)).toBe(true);
      expect(fs.readFileSync(sentinelFile, 'utf-8')).toBe(sentinelContent);

      // The dir build.ts DOES own is still regenerated with fresh generated content.
      const blackholeSkillPath = path.join(root, '.claude', 'skills', 'blackhole', 'SKILL.md');
      expect(fs.existsSync(blackholeSkillPath)).toBe(true);
      expect(fs.readFileSync(blackholeSkillPath, 'utf-8')).toContain(
        '<!-- GENERATED by scripts/build.ts from src/SKILL.md — do not hand-edit -->'
      );
    } finally {
...
describe('opportunistic-fix always-on rule', () => {
  test('RULES_LIST ships opportunistic-fix.md so every platform install gets it', () => {
    expect(RULES_LIST).toContain('opportunistic-fix.md');
  });

  test('source is alwaysApply Cursor MDC and requires filing a forge issue', () => {
    const src = fs.readFileSync(path.join(root, 'src/references/opportunistic-fix.md'), 'utf-8');
    expect(src).toMatch(/^---\n[\s\S]*alwaysApply:\s*true\b/m);
    expect(src).toMatch(/file a forge issue/i);
  });

  test('compileGeminiTree emits opportunistic-fix.md into rules/', () => {
    const destRoot = makeTempDir();
    try {
      compileGeminiTree(
        destRoot,
      ...
      const compiled = fs.readFileSync(path.join(destRoot, 'rules', 'opportunistic-fix.md'), 'utf-8');
      expect(compiled).toMatch(/file a forge issue/i);
      expect(compiled).toContain('GENERATED by scripts/build.ts from src/references/opportunistic-fix.md');
    } finally {
      fs.rmSync(destRoot, { recursive: true, force: true });
    }
  });

  test('AGENTS.md points Cloud/agents at the compiled always-on Cursor and Claude rules', () => {
    const agents = fs.readFileSync(path.join(root, 'AGENTS.md'), 'utf-8');
    expect(agents).toContain('## Opportunistic hygiene (always on)');
    expect(agents).toContain('.cursor/rules/opportunistic-fix.mdc');
    expect(agents).toContain('.claude/rules/opportunistic-fix.md');
  });

  test('Cursor compile keeps alwaysApply: true on opportunistic-fix.mdc', () => {
    const destRoot = makeTempDir();
    try {
      const dest = path.join(destRoot, 'opportunistic-fix.mdc');
      processFile(
        path.join(root, 'src/references/opportunistic-fix.md'),
        dest,
        '.cursor',
        '.cursor/rules/blackhole-vcodes.mdc',
        'cursor'
      ...
      expect(body).toMatch(/^---\n[\s\S]*alwaysApply:\s*true\b/m);
      expect(body).toContain('GENERATED by scripts/build.ts from src/references/opportunistic-fix.md');
    } finally {
      fs.rmSync(destRoot, { recursive: true, force: true });
    }
  });
