...
describe('geminiWorkspaceTreeErrors', () => {
  ...
  test('returns [] on a fully-populated 5-agent workspace tree', () => {
    const destRoot = makeTempDir();
    try {
  ...
  test('reports an agent-count error when fewer than 5 agents are present', () => {
    const destRoot = makeTempDir();
    try {
      populateWorkspaceTree(destRoot);
      ...
      expect(errors.some((e) => e.includes('5 agent'))).toBe(true);
    } finally {
      fs.rmSync(destRoot, { recursive: true, force: true });
    }
  });
...
describe('codexTreeErrors', () => {
  const populateCodexTree = (rootDir: string) => {
    ...
    for (const name of ['coordinator', 'orchestrator', 'planner', 'implementer', 'reviewer']) {
      fs.writeFileSync(
  ...
  test('returns [] on a fully-populated 5-agent codex tree', () => {
  ...
  test('reports an agent-count error when fewer than 5 yaml files are present', () => {
    const rootDir = makeTempDir();
    try {
      populateCodexTree(rootDir);
...
// ('SKILL.md', 'references', '5 agent') to route errors to the correct V-code. These tests
...
describe('codexTreeErrors message contract (verify.ts substring partition)', () => {
  test('agent-count error contains "5 agent"', () => {
