...
| research | `documentation/investigations/research-{concern-slug}.md` — reuses `investigate`'s folder under a distinct filename prefix (ADR-033); not `analysis` type at carry-time, see `doc-governance.md` § Lifecycle Frontmatter's `type` enum |
| plan | `documentation/plans/plan-{concern-slug}.md` (or `documentation/plans/{concern-slug}.md` when the issue title already carries a `Plan` prefix — see `scripts/lib/concern-slug.ts`); its root `documentation/INDEX.md` row is produced by automatic regeneration at carry time from the plan body's own `summary` frontmatter (ADR-031 Phase 2, issue #832), not a per-route staged append — **unconditional** on every issue that reaches Phase 2 plan (Quick, Standard, Skip rationale records included) |
...
the **working copy only** during Handle/Plan turns — including `issue-N-investigation.md` and
`issue-N-analysis.md` notes. It is never the record of truth and does not survive campaign state
rotation. When `docs_governance.write_governance` resolves `true`, `investigator` must also
stage the durable artifact under `.blackhole/staged/<issue>/` (issue #690); `implementer` carries
it into `documentation/investigations/` or `documentation/audits/` in the same PR as plans.
The `documentation/` copy committed in the PR is the durable record; only that copy is subject
