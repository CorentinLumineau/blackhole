...
```json
{
  ...
  "reformulation": {
    ...
    "if_wrong": "What would change in the plan or implementation if an assumption is wrong."
  }
}
```

On `status: ready` for `quick` or `standard` tracks, populate all three `reformulation.*`
strings (`reformulation.understood`, `reformulation.assumed`, `reformulation.if_wrong`) before
returning JSON — the orchestrator posts them as the async veto surface (`confidence-gates.md`,
`phase-plan.md` § Reformulation posting). Omit `reformulation` on `status: blocked` and on
`skip`, `design`, and `brainstorm` ready returns.
