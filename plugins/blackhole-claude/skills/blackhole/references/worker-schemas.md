...
**Install:** shipped, never merged by hand — `bun run build` copies [`templates/hooks/pretooluse/`](../../templates/hooks/pretooluse/hooks.json) into `plugins/blackhole/hooks/` and `plugins/blackhole-claude/hooks/`, so a marketplace install wires `PreToolUse` for `Bash` and `Write|Edit` with no consumer action. Each bundle reads pattern data from its own `hooks/patterns/`; the canonical SSOT — and the path every other install form resolves, since those vendor the repo source — is repo-root `templates/hooks/pretooluse/patterns/`. Deliberately literal, not `plugins/blackhole-claude`-relative: only these two targets receive the tree, so a per-target placeholder would render a path that does not exist on the other three.

**Behavior:** two tiers, because an unattended worker has nobody to ask. A **block** match (destructive command, system path, `../` traversal, write resolving outside the worktree) prints `{"decision":"deny", ...}` and exits `2`. A **warn** match (sensitive filename, force push, registry publish, destructive SQL) prints a `systemMessage` and exits `0`. No match: exit `0`, no output, no record. Patterns are data — adding one is a JSON edit, never a code change.
...
| `tier` | `block` \| `warn` | yes |
| `pattern_id` | matched entry's `id`, or `outside-worktree` \| `pattern-load-failure` | yes |
