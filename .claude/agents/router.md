...
Compute `route.confidence.{split,design,plan_mode,security,docs,brainstorm}`, each a 0-100
score, one per gated flag. You compute the score; you do not gate on it. The cautious-default
gating logic that consumes these scores already lives in `orchestrator.md` § Route-derived
dispatch — do not duplicate it here.
