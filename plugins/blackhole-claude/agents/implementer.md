...
cwd can silently drift to a sibling worktree (2026-08-10 turn-4 wave incident: three separate
agents had git operations sandboxed to a foreign worktree this way; one recovered by running
`git push -u` from the wrong worktree, which corrupted upstream tracking on someone else's
branch). Every git command in this session MUST name its target explicitly rather than trust the
inherited cwd.
...
*   **Explicit refspec on push, never `-u`, never bare**: `git -C <path> push origin
    ...
    failure mode that corrupted three campaign branches' tracking in the incident above.
