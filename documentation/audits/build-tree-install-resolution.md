...
| 6 | `codex-agents/` | **Unconfirmed** — no manifest field references it | `.codex-plugin/plugin.json` (re-read at HEAD) has a `"skills": "./codex-skills/"` key but **no `agents` key at all**; presence is asserted only by build-time/verify-time checks — `scripts/install-verify.ts:17-21` `CODEX_ARTIFACTS` includes `'codex-agents'` in its existence list, and `scripts/build.ts` `codexTreeErrors()` — neither confirms the Codex CLI itself *reads* the directory at install/run time, only that the build+verify toolchain expects it to exist. ADR-038's live install trace 3 (manifest path-reference resolution across every shipped manifest) confirms no manifest anywhere references `codex-agents/`, but the whole-repo Codex git install (`{source: "git", url: <repo>}`) still delivers it to every Codex consumer regardless. | **Shipped-unreferenced** — ADR-038 D3: delivered to every Codex consumer via the whole-repo git install and required by this repo's own Codex build/install checks, though no shipped manifest references it directly |
...
- **Maintainer-surface (2)**: `.claude-plugin/plugin.json` root detached twin (1b, ADR-038 D1);
  `.gemini-plugin/plugin.json` root detached twin (8, ADR-038 D2) — both provably off the
  consumer install path (ADR-038 live install traces 1 and 2), retained as maintainer-tooling
  inputs (release version-parity gate, build-output prefix list, Antigravity build-shape check).
