...
| V-DOC-06 | Incident archaeology in source comments — issue/PR number, "found by review of X", or change-history prose embedded in a comment (a regression test may carry its issue number in the **function name** only). Three settled boundaries: (1) `describe()`/`test()` titles are exempt exactly like function names — a regression test may name its issue in its title; (2) module-header comments: the `#N` tag is archaeology and is removed, the load-bearing rationale stays and is reworded to state the invariant without the citation; (3) markdown prose under `src/**/*.md` and `documentation/**` is exempt — doc prose citations follow the established convention | WARN | reviewer.md §26 (Comment Discipline Audit) |
