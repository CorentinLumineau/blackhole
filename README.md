...
```
`.claude-plugin/marketplace.json` is the catalog the first command reads; its `source` resolves
to the committed `plugins/blackhole-claude/` bundle, which is what gets installed (ADR-009).
Both trees are committed, so no local build is needed to install.
...
### Antigravity / Gemini
`plugins/blackhole/` (redistributable plugin bundle — co-located `plugin.json` + `skills/` +
`rules/`, no `agents/` per the Antigravity plugin schema) is committed in-repo, so a fresh clone
...
```
Maintainers: after editing `src/`, run `bun run build` and commit the changed outputs — the same
flagless build also refreshes `.agents/build/` (workspace customization — 8 agent prompts, rules,
skills; see [AGENTS.md](AGENTS.md) for the roster) and the `.gemini-plugin/` root manifest. Every
git-tracked target is regenerated by plain `bun run build` (tracked ⇒ built by default);
`--gemini` is a deprecated no-op alias kept for one release. `.gemini-plugin/` sits outside the
symlink target above and is a maintainer-surface manifest, not part of the install (ADR-038).
...
```
`codex-marketplace.json` is the catalog; its plugin `source` is the repository itself, so the
install delivers `.codex-plugin/` (manifest, whose `skills` key resolves `codex-skills/`),
`codex-skills/` and `codex-agents/` together — see ADR-038 for `codex-agents/`'s disposition.
