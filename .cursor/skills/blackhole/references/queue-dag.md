...
| `stacked_on` | number \| null | Optional (issue #794, default `null`); the issue number whose branch this issue's PR is based on, when the PR is stacked on a sibling rather than targeting `config.target_branch`. Set once, at stack creation — never inferred afterwards from a live `baseRefName` read, which stops being trustworthy the moment the parent lands |
| `parent_tip_sha` | string \| null | Optional (issue #794, default `null`); `stacked_on`'s branch tip, captured by `git rev-parse` **at stack creation**. It is the `--onto` upstream boundary the post-merge stacked-child repair replays from (`merge-gate.md` § 7). Capturing it at merge time is already too late: once the parent branch is deleted the boundary cannot be reconstructed without reading the merged PR's commit list by hand. Written only by `scripts/stack-repair.ts capture --apply`, run by the orchestrator (`blackhole-state.md` § Single-writer invariant) through the state-write-guard |
...
| `needs_analysis` | boolean | Confidence-gated (`confidence.analysis` vs. `router_confidence_thresholds.analysis`, default 70); cautious default `true` for `size:l`+ or `needs_design: true` issues, else `false` (ADR-010 D1) — triggers investigator · analyze sub-mode (step 6), additionally gated by `autonomy.analyze_routing` |
